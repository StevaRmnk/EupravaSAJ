...
public class GradjaninController {
    ...
    public ResponseEntity<List<Gradjanin>> SviGradjani(){
        ...
        return new ResponseEntity<>(gradjaninService.nadjiSveGradjane(), HttpStatus.OK);

    }

    @GetMapping(value = "/{id}")
    public ResponseEntity<Gradjanin> podaciJednogGradjana(@PathVariable("id")String id){
        return new ResponseEntity<>(gradjaninService.nadjiJednogGradjanina(id),HttpStatus.OK);
    }
